...
class Scraper:
    ...
    def load_backup(self, filename):
        ...
        try:
            if self.verbose: print(f"Loading backup: {filename}")
            
            # load pickle file in the backups folder
            pickle_file = os.path.join(os.getcwd(), "backups", f"{filename}.pkl")
            
            if self.verbose: print(f"Searching backup in: {pickle_file}")
            if self.verbose: print(f"Backup is existing: {os.path.exists(pickle_file)}")
            
            # pickle_file = os.getcwd() + f"""/{filename}.pkl"""
            if os.path.exists(pickle_file):
                if self.verbose: print(f"Backup found: {filename}")
                open_parsed_df = pd.read_pickle(pickle_file)

                # parsed_articles = open_parsed_df.to_dict('records')
                
                if self.verbose: print(f"Backup loaded: {filename}")
